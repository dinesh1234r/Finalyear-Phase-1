...
function GetAllDoctor() {
  ...
  const fetchdetails=async()=>{
    ...
    const response=await axios.post('https://finalyear-phase-1-backend.onrender.com/admin/getalldetailsofdoctor',{Admin},{
      headers:{
  ...
  const handlesubmit=async()=>{
    const response=await axios.post("https://finalyear-phase-1-backend.onrender.com/admin/deletedetail",{Medical_License_Number:medicalnum},{
      headers:{
        'Authorization':`Bearer ${localStorage.getItem('jwt')}`,
        'Content-Type': 'application/json'
      }
