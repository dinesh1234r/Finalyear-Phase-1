...
function GetAllNurse() {
  ...
  const fetchdetails=async()=>{
    ...
    const response=await axios.post('https://finalyear-phase-1-backend.onrender.com/admin/getalldetailsofnurse',{Admin},{
      headers:{
  ...
  const handlesubmit=async()=>{
    const response=await axios.post("https://finalyear-phase-1-backend.onrender.com/admin/deletedetailnurse",{Medical_License_Number:medicalnum},{
      headers:{
        'Authorization':`Bearer ${localStorage.getItem('jwt')}`,
        'Content-Type': 'application/json'
      }
